...
namespace TifSnippetApp.Services
{
    public class SnippetService
    {
        ...
        /// <summary>
        /// Resolves the full path to an image file, falling back to a .jpg extension
        /// if the original path (e.g. .png) does not exist on disk.
        /// </summary>
        private string ResolveImagePath(string filename)
        {
            var primary = Path.Combine(_datasetPath, filename);
            if (File.Exists(primary)) return primary;

            // Try swapping the extension to .jpg
            var ext = Path.GetExtension(filename);
            if (!string.Equals(ext, ".jpg", StringComparison.OrdinalIgnoreCase))
            {
                var fallback = Path.Combine(_datasetPath,
                    Path.GetFileNameWithoutExtension(filename) + ".jpg");
                if (File.Exists(fallback)) return fallback;
            }

            return primary; // return original so caller's File.Exists check shows it missing
        }
        ...
        public async Task<List<SnippetInfo>> GetSnippetsAsync(int startIndex, int count)
        {
            ...
            for (int i = startIndex; i < _records.Count; i++)
            {
                ...
                var imagePath = ResolveImagePath(record.Filename);
                ...
                try
                {
                    ...
                    var rectRight = (int)Math.Min(image.Width, maxX);
                    var rectBottom = (int)Math.Min(image.Height, maxY);
        ...
        public async Task<string?> GetSnippetImageAsync(int index, bool expanded)
        {
            ...
            var maxXExpanded = minX + width;
            var maxYExpanded = minY + height;

            // Clamp to image bounds
            var rectX = (int)Math.Max(0, minX);
            var rectY = (int)Math.Max(0, minY);
            var rectRight = (int)Math.Min(image.Width, maxXExpanded);
            var rectBottom = (int)Math.Min(image.Height, maxYExpanded);
            var rectWidth = rectRight - rectX;
            var rectHeight = rectBottom - rectY;
